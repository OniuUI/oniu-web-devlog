...
export default function RoomManagement({ onJoinRoom }: RoomManagementProps) {
  ...
  async function createRoom() {
    ...
    const roomTitle = newRoomTitle.trim() || roomName
    setNewRoomName('')
    setNewRoomTitle('')
    setShowCreateRoom(false)
    saveAcceptedRoom(roomName, roomTitle)
        ...
        <div className="rounded-2xl bg-neutral-950/40 p-3 ring-1 ring-white/10">
          ...
          <div className="space-y-2">
            <input
              ...
              className="w-full rounded-lg bg-neutral-950/50 px-2 py-1.5 text-[11px] text-neutral-200 ring-1 ring-white/10 placeholder:text-neutral-500 focus:outline-none focus:ring-white/20"
              maxLength={32}
            />
            <input
              value={newRoomTitle}
            ...
            <div className="space-y-1 max-h-40 overflow-auto">
              {acceptedRooms.map((r) => (
                <button
                  ...
                  type="button"
                >
                  <span className="truncate">{r.title || r.room}</span>
